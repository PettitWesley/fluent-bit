                              ...
                              struct flb_config *config, void *data)
{
    ...
    if (ctx->disable_sequence_token == FLB_TRUE) {
        flb_plg_warn(ctx->ins, "Enabling multi-worker support...");
    } else {
        flb_plg_error(ctx->ins, "This custom version of AWS for Fluent Bit only works with "
                      "`disable_sequence_token On`. If you do not need that feature "
                      " please use our normal public release.");
        return -1;
    }
